...
export function createPostgresApprovalStore(db: SqlClient): AsyncApprovalStore {
  return {
    async put(request) {
    ...
    async get(id) {
      const result = await db.query<Row>('SELECT id, action, target, requested_by, state, created_at, expires_at FROM approval_requests WHERE id=$1', [id]);
      return result.rows[0] ? mapRow(result.rows[0]) : undefined;
    },
    async list() {
      const result = await db.query<Row>('SELECT id, action, target, requested_by, state, created_at, expires_at FROM approval_requests ORDER BY created_at DESC');
